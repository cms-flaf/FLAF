...
if __name__ == "__main__":
    ...
    for unc_src, unc_src_dict in joint_dict.items():
        ...
        for lep_cat, lep_cat_dict in unc_src_dict.items():
            multiplicities = list(np.unique([int(key.split('_')[-1]) for key in lep_cat_dict.keys()]))
            
            if lep_cat not in this_unc_dict.keys():
                this_unc_dict[lep_cat] = {}

            this_lep_cat_dict = this_unc_dict[lep_cat]
            weights_before = np.array([lep_cat_dict[f'weight_noBtag_ncentralJet_{m}'] for m in multiplicities])
            weights_after = np.array([lep_cat_dict[f'weight_total_ncentralJet_{m}'] for m in multiplicities])
            zero_weight_mask = np.logical_or(weights_after == 0.0, weights_before == 0.0)
