...
class DYtautauStitcher(MCStitcher):
    ...
    else 0) from ``TauTauInfo_passFilter`` when the anaTuple stores it, otherwise from the
    strict tau-tau identification in ``FLAF/include/GenProcess/DY.h``.
    """
    ...
    def defineVariables(self, df):
        df = defineFromStoredOrExpression(
            df,
            "DY_tautau_filter",
            stored="TauTauInfo_passFilter",
            stored_expression="TauTauInfo_passFilter ? 1 : 0",
            expression="gen_process::dy::identifyTauTau(GenPart_pt, GenPart_eta, "
            "GenPart_phi, GenPart_mass, GenPart_pdgId, GenPart_statusFlags, "
