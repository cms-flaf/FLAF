...
| `global.yaml` | both layers | Global settings: anaTuple/histTuple definitions, corrections, payload producers, signal types. For 2024/2025 this also holds `shared_mc`, `weight_base_branch` (`weight_base` for a single year, `weight_base_cmb` for 24+25) and, on 2025, `reuse_mc_from_era`. |
