...
def draw_ratio(ax_ratio, bin_edges, data_vals, data_errs,
    ...
    with np.errstate(divide='ignore', invalid='ignore'):
        ...
        ratio_err = np.divide(data_errs, total_mc_vals,
    ...
    #     ax=ax_ratio
    # )
    # # Disegna solo i punti non blindati
    # hep.histplot(ratio[mask], bins=bin_edges, yerr=ratio_err[mask],
    #              histtype="errorbar", color="black", ax=ax_ratio)

    ax_ratio.axhline(1.0, color="black", linestyle="--", linewidth=1.0)
    delta = 0.5  # valore di fallback
    if len(ratio[mask]):
        delta = np.abs(ratio[mask] - 1).max()

    # Centra y_min e y_max attorno a 1.0
    y_max = round(1 + delta, 2)
    y_min = round(1 - delta, 2)
    # y_max = round(ratio[mask].max() if len(ratio[mask]) else 1.5, 2)
    # # y_min = ratio[mask].min() if len(ratio[mask]) else 0.5
    # y_min = round((2-(y_max)), 2)
    print(f"y_max = {y_max}")
    print(f"y_min = {y_min}")
    ax_ratio.set_ylim(0.5, 1.5)
    ax_ratio.yaxis.set_ticks(np.arange(0.5, 1.5, 0.15))
...
def plot_histogram_from_config(
    ...
    ratio_plot = bool(wantData and wantRatio and stacked)
    ...
    # -------------------------
    # Modalità "compare" (region overlay)
    # -------------------------
    if compare_mode:
        linestyle_cycle = ['-', '--', ':', '-.']
        region_styles = {region: linestyle_cycle[i % len(linestyle_cycle)] for i, region in enumerate(histograms_dict.keys())}

        # scegli binning di riferimento dalla prima regione/primo contrib
        ...
        for region, hist_dict in histograms_dict.items():
            for contrib_name, hist in hist_dict.items():
                vals, _, _, _ = get_hist_arrays(hist, divide_by_bin_width)
                config_input = processes_dict.get(contrib_name, {})
                color_name = config_input.get("color_mplhep", "black")
                # label = f"{region.split('/')[-1]}"
                label = config_input.get("name", contrib_name)
                hep.histplot(
                    vals, bins=bin_edges, histtype="step", color=color_name,
                    linestyle=region_styles.get(region, '-'), linewidth=3, label=label, ax=ax
                )
        ...
        for var, total_hist in histograms_dict.items():
            ...
            hep.histplot(
                values, bins=bin_edges, histtype="step",
                 color=style[1], linewidth=linewidth, label=var, ax=ax, linestyle=style[0],alpha=alpha,
            )
    ...
    # Limiti Y (se abbiamo MC, usa la somma per un margine confortevole)
    # try:
    y_max = None
    if mc_hists_withMinor:
        ...
        vals, errs, bin_edges, bin_widths = get_hist_arrays(first_hist, False)
        y_max = np.max(vals)  if vals is not None and len(vals) else None
    # print(y_max)
    if y_max is not None and np.isfinite(y_max) and y_max > 0:
            max_factor = (100**(hist_cfg.get("max_y_sf", 1.)) if wantLogY else hist_cfg.get("max_y_sf", 1.))
            ax.set_ylim(top=y_max * max_factor) # hist_cfg.get(max_y_sf)

    # if (stacked or compare_mode or not stacked) and mc_hists:
    #     x_min = bin_edges[0] * 0.99
    #     x_max = bin_edges[-1] * 1.01
    ...
    # Ratio plot (Data/MC)
    # -------------------------
    if ratio_plot and not compare_mode:
