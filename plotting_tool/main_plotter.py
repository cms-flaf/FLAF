...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--period", required=True, type=str, default="Run3_2022", help="Run era"
    )
    parser.add_argument(
        "--var", required=True, type=str, default="m_mumu", help="Variable to plot"
    )
    parser.add_argument(
        "--channel", required=False, type=str, default="muMu", help="Analysis channel"
    )
    parser.add_argument(
        "--region",
        required=False,
        type=str,
        default="OS_Iso",
        help="region (QCD, Mass..)",
    )
    parser.add_argument(
        "--subregion", required=False, type=str, default=None, help="subregion"
    )
    parser.add_argument(
        "--category",
        required=False,
        type=str,
        default="inclusive",
        help="Analysis category (res1/2b, boosted, ggH/VBF,..)",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        required=False,
        type=str,
        help="specific path for histograms",
        default=None,
    ...
    parser.add_argument(
        ...
        required=False,
        type=str,
        help="comma-separated subcategory to compare",
    )
    parser.add_argument(
    ...
    pre_path = (
        f"{args.channel}/{args.region}/{args.category}"
        if args.pre_path is None
        else args.pre_path
    )  # patch for current bbtt shapes
    if args.subregion:
    ...
    if regions_to_compare:
        comparison_paths = [
            f"{pre_path}/{reg}" for reg in regions_to_compare
        ]  # structure has the sub-regions to compare as final folders containing the histos: e.g. channel-region-category-subregion(s)
        ...
        if all(path in hists_to_plot_binned for path in comparison_paths):
            ...
            plot_histogram_from_config(
                ...
                wantRatio=args.wantRatio,  # should be false in compare mode actually, as ratio plot makes no sense in this case.
                category=args.category,
                channel=args.channel,
            )
        else:
        ...
        main_region_data = {
            contrib: hists_to_plot_binned[pre_path][contrib]
            for contrib in all_contributions
            if contrib in hists_to_plot_binned[pre_path]
        }
