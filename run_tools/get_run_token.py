...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Request a start token from the token server"
    )
    ...
    parser.add_argument(
        "--timeout", type=float, default=3600, help="max wait time in seconds"
    )
    parser.add_argument(
        "--retry-interval",
        type=float,
    ...
    if not get_token(
        args.server, args.port, args.path, args.timeout, args.retry_interval
    ):
        sys.exit(1)
